...
<body>
	...
	<?php
		...
		while($row=mysqli_fetch_array($result)){
			...
			$attend=mysqli_fetch_array(mysqli_query($db,"select * from $subject where ID='{$_SESSION['ID']}';"));
			for($date=2;$date<mysqli_num_fields($result);$date++){
				$percentage[4][$i]+=$attend[$columns[$date]];
			}
			$percentage[4][$i]=($percentage[4][$i]*100)/(mysqli_num_fields($result)-2);
			$i++;
		}
		...
		while($row=mysqli_fetch_array($result)){
			...
			$attend=mysqli_fetch_array(mysqli_query($db,"select * from $subject where ID='{$_SESSION['ID']}';"));
			for($date=5;$date<mysqli_num_fields($result);$date++){
				$percentage[5][$i]+=$attend[$columns[$date]];
			}
			$percentage[5][$i]=($percentage[5][$i]*100)/(mysqli_num_fields($result)-2);
			$i++;
		}
		...
		<tr class="row">
			...
			<th class=" text-center">AOA</th>
			<th class=" text-center">OSTL</th>
