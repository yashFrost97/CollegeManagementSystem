		...
		<div class="row">
			...
			<form action="studentLogin.php" method="post">
				<div class="login-form">	
					...
					<div class="form-group">
						<button class="btn btn-lg btn-danger btn-block" type="submit" name="slogin"><i class="fa fa-user" aria-hidden="true"></i>Login!</button>
					</div>
				</div>
			</form>
		</div>
...
<?php
	if(isset($_POST['slogin'])){
			...
			$Password = $_POST['Password'];
				if(mysqli_num_rows(mysqli_query($db,"Select * from te where ID='{$ID}';"))>0){
					?>
					<?php
					if(mysqli_num_rows(mysqli_query($db,"select * from te where ID='{$ID}' AND Password='{$Password}';"))>0){
						$row=mysqli_fetch_array(mysqli_query($db,"select * from te where ID='{$ID}';"));
						
						$_SESSION['yearr']=$row['YEAR'];
						$_SESSION['ID']=$row['ID'];
